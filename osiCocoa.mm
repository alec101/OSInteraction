...
void osiCocoa::makeCurrent(osiWindow *w) {
  ...
  //https://developer.apple.com/library/mac/documentation/GraphicsImaging/Reference/CGL_OpenGL/Reference/reference.html#//apple_ref/doc/uid/TP40001186-CH3g-570567
  must use this, CGL is it's core stuff
  the makeCurrent can return an error if not possible to activate a context on a specific window...
  current function does not return shit...
  at this moment, the context is tied to the view... and each window has a view... this is kinda messi ffs

  void *clg= CGLContextObj(); //??? something like this
