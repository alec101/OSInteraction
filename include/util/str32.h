...
public:
  ...
  void insertStr(const char32_t *in_str, int32_t in_pos= -1); // <in_str>- str to insert; <in_pos>- insert position (-1= at the end of the str)
  ...
  inline operator char32_t *() { return d; }           // returns pointer to internal data (d)
  //operator char16_t *() { return convert16(); } // converts to UTF-16. returned pointer is part of the class, no need for dealloc
  //operator uint8_t *()  { return convert8(); }  // converts to UTF-8. returned pointer is part of the class, no need for dealloc

  inline operator bool() const     { return (len? true: false); } // returns true if string has any characters
  ...
  inline bool operator!() { return nrUnicodes? true: false; };
};
